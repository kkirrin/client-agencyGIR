...
export default function WorkerItem({ setWorkers, workers, worker, displayedDays, handleClick, handleClickNote }) {
  ...
  return (
    <>
        {/* Проверка на то, что массив не пустой и в нем есть значения */}
        {/* Иначе должно быть пустой элемент для запонения  */}
        {
       ...
       {
        workers.length > 0 && worker.name == '' && (
            <motion.div
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }} 
                exit={{ opacity: 0, y: 20 }}
                transition={{ duration: 0.5 }} key={worker.id}
                className={`${styles.workers_item_wrapper} ${styles.workers_item_wrapper_empty}`}
            >
